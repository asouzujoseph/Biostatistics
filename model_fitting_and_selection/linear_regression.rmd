...
from the regression model. Control the type I error at  = .10 at each stage. Which
...
* How confident do you feel about the model you selected? Discuss.<br>
```{r}
library("pastecs")
library("tidyverse")
...
```{r}
desc.df <- stat.desc(df[,c("AGE","LWT","RACE","FTV")], basic =T, desc=T)
desc.df
```
...
mod.aictab=aictab(cand.set = mod.list) ##, modnames = donner.modnames)
mod.aictab
```
<br>
The model fitted by forward selection technique using AICc score is **BWT ~ AGE + LWT + RACE**. <br>
Alhough this model did not have the lowest AICc score but then it is logical to include the mother's age as one of the predictors of the birth weight of a baby because it is known that maternal age influences the birth weight of babies as seen in published articles <br>
...
```
<br>
The plot shows that women with urinary tract infections had babies with lower birth weight compared to women without urinary tract infections.
<br>
```{r}
ggplot(df,aes(FTV,BWT,color = FTV))+geom_boxplot() + theme_classic()
```
...
**Model building using AIC and AIC weights**
```{r}
mod.list <- list()
...
This model is slightly different from the model fitted with forward selection technique and AICc.  Age has a p -value greater than 0.1 but we will include it in the model because it is known that maternal age affects birth weight of babies ( a condition known as Macrosomia) <br
